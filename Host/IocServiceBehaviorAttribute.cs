namespace Softweyr.Infrastructure.Wcf
{
    ...
    /// <summary>
    /// Attribute for marking a service for dependency injection using the inversion of control container.
    /// </summary>
    public class IocServiceBehaviorAttribute : Attribute, IServiceBehavior
    {
        /// <summary>
        ...
        {
            foreach (var cbd in serviceHostBase.ChannelDispatchers)
            {
                var cd = cbd as ChannelDispatcher;
                if (cd != null)
                {
                    foreach (EndpointDispatcher ed in cd.Endpoints)
                    {
                        var iocInstanceProvider = new IocInstanceProvider(serviceDescription.ServiceType);
                        ed.DispatchRuntime.InstanceProvider = iocInstanceProvider;
                    }
                }
            }
        }
